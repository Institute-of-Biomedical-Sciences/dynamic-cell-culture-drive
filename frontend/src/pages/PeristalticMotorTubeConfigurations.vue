<template>
  <div class="h-full flex flex-col min-h-0">
    <div class="page-header flex-shrink-0">
      <h1 class="page-title">
        <span class="text-muted-color">Peristaltic Tube Configurations</span>
      </h1>
    </div>
    <div class="flex w-full flex-1 min-h-0">
      <Card style="height: 81.5vh" class="w-full calibration-card">
        ...
        <template #content>
          <div class="mb-2">
            <div
              class="text-md border-round-lg p-2 bg-gray-100 text-gray-500 inline-block shrink-to-fit"
            >
              The preset tube configuration mL/rotation is calculated by:
              <span class="ml-2 inline-math" aria-hidden="true">
                <math
                ...
                >
                  <mfrac>
                    <mrow>
                      <mrow><mn>51</mn><mo>/</mo><mn>4</mn></mrow>
                      <mo>&#x00D7;</mo>
                      <msup><mi>&#x03C0;</mi><mn>2</mn></msup>
                      <mo>&#x00D7;</mo>
                      <msup><mi>diameter</mi><mn>2</mn></msup>
                    </mrow>
                    <mn>1000</mn>
                  </mfrac>
                </math>
            ...
            <Column field="preset" header="Type">
              <template #body="slotProps">
                <Tag
                  ...
                  :severity="slotProps.data.preset ? 'success' : 'warn'"
                />
              </template>
            </Column>
            <Column
  ...
  >
    <div class="flex flex-col my-2 gap-2">
      <FloatLabel class="w-full" variant="on">
        <Select
          class="w-full"
          v-model="newTubeConfiguration.preset"
      ...
      <div class="flex flex-col mb-2 gap-2">
        <FloatLabel class="w-full" variant="on">
          <InputNumber
            ...
            :maxFractionDigits="3"
            class="w-full"
            v-model="newTubeConfiguration.diameter"
          />
      ...
      <div class="flex flex-col mb-2 gap-2">
        <FloatLabel class="w-full" variant="on">
          <InputNumber
            ...
            :maxFractionDigits="3"
            class="w-full"
            v-model="newTubeConfiguration.flow_rate"
          />
    ...
    <div v-else>
      <div class="flex flex-col mb-2 gap-2">
        <FloatLabel class="w-full" variant="on">
          <InputNumber
            class="w-full"
            v-model="newPeristalticCalibration.duration"
          />
      ...
      <div class="flex flex-col mb-2 gap-2">
        <FloatLabel class="w-full" variant="on">
          <InputNumber
            class="w-full"
            v-model="newPeristalticCalibration.low_rpm"
          />
          <label>Low RPM</label>
        </FloatLabel>
      </div>
      <div class="flex flex-col mb-2 gap-2">
        <FloatLabel class="w-full" variant="on">
          <InputNumber
            class="w-full"
            v-model="newPeristalticCalibration.high_rpm"
          />
      ...
      <div class="flex flex-col mb-2 gap-2">
        <FloatLabel class="w-full" variant="on">
          <InputNumber
            ...
            :maxFractionDigits="3"
            class="w-full"
            v-model="newPeristalticCalibration.low_rpm_volume"
          />
      ...
      <div class="flex flex-col mb-2 gap-2">
        <FloatLabel class="w-full" variant="on">
          <InputNumber
            ...
            :maxFractionDigits="3"
            class="w-full"
            v-model="newPeristalticCalibration.high_rpm_volume"
          />
    ...
    <template #footer>
      ...
      />
      <Button label="Add" severity="primary" @click="addTubeConfiguration()" />
    </template>
  </Dialog>
  <Toast></Toast>
</template>
...
import {
  type TubeConfiguration,
  type PeristalticCalibration,
  type ApiResponse,
} from "../api";
...
const onRowEditSave = async (event: any) => {
  ...
  try {
    ...
    if (newData.preset) {
      ...
      let calibrationData = calibrations.value.find(
        (calibration) => calibration.name === newData.name,
      );
      if (calibrationData) {
...
const addTubeConfiguration = async () => {
  try {
    ...
    if (newTubeConfiguration.value.preset) {
      response = await peristalticMotorApi.saveTubeConfiguration(
        newTubeConfiguration.value,
      );
    } else {
      response = await peristalticMotorApi.savePeristalticCalibration({
        ...newPeristalticCalibration.value,
      });
    }
    if (response.success) {
      fetchTubeConfigurations();
